...
class MatchingGraph {
   public:
    ...
    void add_boundary_edge(
        ...
        const std::vector<size_t>& observables,
        const std::vector<pm::ImpliedWeightUnconverted>& implied_weights_for_other_edges = {});
    void update_edges(const std::vector<size_t>& u_nodes, const std::vector<size_t>& v_nodes, const std::vector<signed_weight_int>& weights);
    void update_boundary_edges(const std::vector<size_t>& u_nodes, const std::vector<signed_weight_int>& weights);
